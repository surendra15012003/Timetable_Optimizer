...
export class TimetableSmtlib2Converter {
    ...
    constructor(
        timetable: GenericTimetable,
        total_half_hour_slots: number,
        ...
        let time_str_vals: Array<string> = Array.from(
            new Array(total_half_hour_slots),
            (_: number, i: number) => {
    ...
    update_z3_weeksolve_output(buffer: string) {
        ...
        binary.split('').forEach((c: any, idx: number) => {
            if (c === '1') this.weeks_to_simulate.add(idx + 1);
        });
    ...
    generateTimetableSolveSmtLib2String(randomize: boolean = true): string {
        ...
        this.gt.modules.forEach((mod: Module) => {
            Object.keys(mod.lessons).forEach((lessonType: string) => {
                const lessons_of_lessontype: Array<Lesson> = mod.lessons[lessonType];
                const slot_constraints: Array<SlotConstraint> = this.module_lessons_to_slotconstraints(
                    mod,
                    lessons_of_lessontype
                );
        ...
        if (this.gt.constraints.workloadActive) {
            ...
            this.z3tt.set_boolean_selector_costs(
                optional_workloads,
                compulsory_workload_sum,
                this.gt.constraints.minWorkload,
                this.gt.constraints.maxWorkload
            );
        }
        ...
        if (this.gt.constraints.freeDayActive || this.gt.constraints.specificFreeDaysActive) {
            ...
            if (this.gt.constraints.freeDayActive) {
                ...
                // this.z3tt.add_constraints_fulfil_only_one(slot_constraints);
                this.z3tt.add_constraints_fulfil_exactly_N(
                    slot_constraints,
                    this.gt.constraints.numRequiredFreeDays
                );
            }
        ...
        if (this.gt.constraints.lunchBreakActive) {
            ...
            slot_constraints.forEach((sc: SlotConstraint) => {
                this.z3tt.add_negativevalue_constraint_to_N_consecutive(
                    sc,
                    this.gt.constraints.lunchHalfHours
                );
            });
        }
        ...
        if (this.gt.constraints.timeConstraintActive) {
            const slot_constraint:
                | SlotConstraint
                | undefined = this.generate_timeconstraint_slotconstraint();
            ...
            lesson.start_end_times.forEach(([g_start_time, g_end_time], idx) => {
                ...
                if (this.weeks_to_simulate.size === 0) {
                    ...
                    const weeks_to_sim = weeks_for_lesson.filter((week: number) =>
                        this.weeks_to_simulate.has(week)
                    );
                    for (let week of weeks_to_sim) {
                        console.log('Simulating week ' + week);
                        const start_time = this.generic_time_to_z3_time(
                            g_start_time,
                            lesson.days[idx],
                            week - 1
                        );
                        const end_time = this.generic_time_to_z3_time(
                            g_end_time,
                            lesson.days[idx],
                            week - 1
                        );
    ...
    generate_free_day_slotconstraints(): Array<SlotConstraint> {
        ...
        for (let day = 0; day < DAYS - 1; day++) {
            ...
            const sc: SlotConstraint = {
                start_end_times: start_end_idxs,
                who_id: who_id,
                who_id_string: name,
            };
            scs.push(sc);
    ...
    generate_lunch_break_slotconstraints(): Array<SlotConstraint> {
        ...
        for (let day = 0; day < DAYS; day++) {
            ...
            for (let week of Array.from(this.weeks_to_simulate)) {
                ...
                const sc: SlotConstraint = {
                    start_end_times: [[startidx, endidx]],
                    who_id: UNASSIGNED,
                    who_id_string: 'UNASSIGNED',
                };
                scs.push(sc);
            }
        }
    ...
    generic_time_to_z3_time(timeval: Date, day: string, week: number = 0): number {
        ...
        if (hour < this.start_hour || hour > this.end_hour) {
            throw new Error(
                `Lesson either starts before start_hour ${hour} < ${this.start_hour} or ends after end_hour ${hour} > ${this.end_hour}`
            );
        } else {
